...
export function fakeApi(overrides: Record<string, any> = {}) {
  return {
    ...
    listDocuments: vi.fn(async () => ({ results: [] })),
    getDocumentSuggestions: vi.fn(async () => ({ tags: [], correspondents: [] })),
    getDocumentNotes: vi.fn(async () => []),
    deleteDocumentNote: vi.fn(async () => []),
    getDocumentMetadata: vi.fn(async () => ({ original_checksum: "x" })),
    getDocumentHistory: vi.fn(async () => []),
